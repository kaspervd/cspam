...
def correctPB(imgname, freq, phaseCentre=None):
    """Given an image "img" create the "img.pbcorr" which
    has each pixel corrected for the primary beam effect.
    """
    img = ia.open(imgname)
    ...
    if phaseCentre == None:
        coord = np.ndarray(len(ia.shape()))
        coord[0] = ia.shape()[0]/2.
        coord[1] = ia.shape()[1]/2.
        phaseCentre = ia.toworld(coord)['numeric']
    beam = np.zeros(ia.shape())
    for i, _null in np.ndenumerate(beam):
        # get distance from phase centre
        coord = ia.toworld(i)['numeric']
        d = au.angularSeparation(phaseCentre[0]*180/np.pi, phaseCentre[1]*180/np.pi, coord[0]*180/np.pi, coord[1]*180/np.pi)
        au.close()
        # from http://www.aips.nrao.edu/cgi-bin/ZXHLP2.PL?PBCOR
        d = d * 60 * freq/1.e3
        beam[i] = 1 + (parm[0]/10**3)*d**2 + (parm[1]/10**7)*d**4 + \
             (parm[2]/10**10)*d**6 + (parm[3]/10**13)*d**8
    # write new image
    impbcor(imagename=imgname, pbimage=beam, outfile=imgname+'.pbcorr',\
            mode='divide')

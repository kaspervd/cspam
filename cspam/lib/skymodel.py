...
#        nvss_file_name = catalog_path_e + 'SU00.0050'
  ...
  if ( dec + radius >= 90. ) or ( dec - radius <= -90. ):
    ...
    cos_dec = math.cos( math.radians( max( [ abs( dec + radius ), abs( dec - radius ) ] ) ) )
...
# radius <= 90 degrees
  ...
  old_source_list = []
  source_file_name_e = os.path.expandvars( source_file_name )
  if os.path.isfile( source_file_name_e ):
    if append:
...
#  w_limit = ( 3600. / ( cell_size / 2. ) ) * ( 360. / ( 2. * pi ) ) * sqrt( 2. * max_smearing * w_max )
  ...
  pb_facet_file_name_e = os.path.expandvars( pb_facet_file_name )
  if os.path.isfile( pb_facet_file_name_e ):
  ...
  if os.path.isfile( temp_facet_file_name ):
  ...
  if os.path.isfile( cpb_facet_file_name ):
...
def define_o_facets( uv, flux_min, field_ratio = 2., o_facet_size = None,
  ...
  # generate source catalog
  radec = get_radec( uv )
  file_prefix_e = os.path.expandvars( file_prefix )
  ...
  o_facet_file_name_e = os.path.expandvars( o_facet_file_name )
  field_size = restore_parameter( uv, 'field_size' )
  ...
  if ( ( not os.path.isfile( catalog_name ) ) and ( not append ) ):
  ...
  if os.path.isfile( o_facet_file_name_e ):
    if append:
...
def define_a_s_facets( uv, a_facet_size = 128., s_ang_facet_size = 1.,
  ...
  a_catalog_name = os.path.expandvars( file_prefix ) + 'A_TEAM.CAT'
  if os.path.isfile( a_catalog_name ):
  ...
  as_facet_file_name = os.path.expandvars( as_facet_file_name )
  if os.path.isfile( as_facet_file_name ):
  ...
  a_facet_file_name_e = os.path.expandvars( a_facet_file_name )
  ...
  s_facet_file_name_e = os.path.expandvars( s_facet_file_name )
...
def get_SH_model_flux_spix( cal_name, frequency, model = None ):
  if ( model is None ):
    ...
    models = [
        ...
        [ log10( 23.314 ), -0.843, -0.461,  0.249, -0.083 ] ] # 3C468.1
...
def get_SH_model_flux_spix_array( cal_name, frequency, times = 10000 ):
  ...
  models = [
      ...
      [ 23.314, -0.843, -0.461,  0.249, -0.083 ] ] # 3C468.1
  dmodels = [
      ...
      [ 0.716, 0.039, 0.049, 0.083, 0.058 ] ] # 3C468.1
  if ( not cal_name in calibrators ):
    raise error( 'calibrator %s not in Scaife & Heald (2012)' % ( cal_name ) )
  index = calibrators.index( cal_name )
